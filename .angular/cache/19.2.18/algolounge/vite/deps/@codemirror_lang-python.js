...
var TokenCache = class {
  ...
  updateCachedToken(token, tokenizer, stack) {
    ...
    if (token.value > -1) {
      ...
      for (let i = 0; i < parser2.specialized.length; i++) if (parser2.specialized[i] == token.value) {
...
var Parse = class {
  ...
  advanceStack(stack, stacks, split) {
    ...
    if (this.fragments) {
      ...
      for (let cached = this.fragments.nodeAt(start); cached; ) {
        ...
        if (match > -1 && cached.length && (!strictCx || (cached.prop(NodeProp.contextHash) || 0) == cxHash)) {
          ...
          if (verbose) console.log(base + this.stackID(stack) + ` (via reuse of ${parser2.getName(cached.type.id)})`);
          return true;
        }
    ...
    if (defaultReduce > 0) {
      ...
      if (verbose) console.log(base + this.stackID(stack) + ` (via always-reduce ${parser2.getName(
        defaultReduce & 65535
        /* Action.ValueMask */
      )})`);
      return true;
    ...
    for (let i = 0; i < actions.length; ) {
      ...
      if (verbose) console.log(base + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser2.getName(
        action & 65535
        /* Action.ValueMask */
