  ...
  ngOnInit() {
    ...
    this.route.paramMap.pipe(takeUntil(this.destroy$)).subscribe((params) => {
      ...
      if (name) {
        ...
        // Check completion immediately (may refresh after completions load)
      ...
      if (this.executionResult.passedCount === this.executionResult.totalCount && this.executionResult.totalCount > 0) {
        ...
        if (!this.isCompleted) {
          ...
          this.completionService
            .markCompleted(this.currentQuestionFilename)
            .pipe(takeUntil(this.destroy$))
            .subscribe({
              error: (err) => console.error('Error marking question as completed:', err),
            });
        }
